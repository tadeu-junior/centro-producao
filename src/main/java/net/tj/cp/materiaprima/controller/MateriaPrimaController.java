...
public class MateriaPrimaController {
	...
	public String searchMateriaPrima(@QueryValue(MateriaPrimaController.LIMITE_MINIMO) @Nullable Integer limiteMinimo){
		return materiaPrimaService.searchMateriaPrimaQuantidadeMinima(limiteMinimo);
	}
	...
	public HttpResponse update (@Body @Valid MateriaPrima materiaPrima){
		...
		logger.info(materiaPrima.toString());
		return HttpResponse.ok(materiaPrima).header(HttpHeaders.LOCATION, createLocation(materiaPrima).getPath());
	}
